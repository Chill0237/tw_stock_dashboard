document.addEventListener("DOMContentLoaded", () => {
    ...
    function buildCards() {
        ...
        Object.entries(METRIC_CONFIGS).forEach(([key, cfg]) => {
            ...
            card.innerHTML = `
                <div class="p-3 border border-slate-700 dark:border-slate-200 h-full flex flex-col dark:bg-white">
                    <div class="flex justify-between items-baseline mb-2 pb-1 border-b border-slate-700 dark:border-slate-200">
                    ...
                    <table class="table-fixed w-full text-xs">
                        <thead>
                            <tr class="text-slate-600 dark:text-gray-400 border-b border-slate-700 dark:border-slate-200">
                        ...
                        <tbody id="body-${key}" class="divide-y divide-slate-800 dark:divide-slate-100">
                            <tr><td colspan="3" class="text-center py-3 text-slate-600 dark:text-gray-600 text-2xs">loading</td></tr>
                        </tbody>
                    </table>
                </div>`;
        ...
        buttons.forEach(btn => {
            btn.addEventListener("click", () => {
                buttons.forEach(b => {
                    ...
                    b.classList.add("border-transparent", "text-slate-400", "dark:text-gray-600");
                });
    ...
    async function fetchAndRender(option) {
        ...
        try {
            ...
            Object.keys(METRIC_CONFIGS).forEach(key => {
                ...
                if (tbody) tbody.innerHTML = `<tr><td colspan="3" class="text-center py-3 text-rose-600 dark:text-rose-600 text-2xs">error</td></tr>`;
            });
        }
    }
    ...
    function renderTable(metricKey, data) {
        ...
        if (!data || data.length === 0) {
            tbody.innerHTML = `<tr><td colspan="3" class="text-center py-3 text-slate-600 dark:text-gray-600 text-2xs">no signal</td></tr>`;
            return;
        }
        ...
        data.forEach(item => {
            ...
            tr.className = "hover:bg-slate-800 dark:hover:bg-slate-100";
            ...
            tr.innerHTML = `
                ...
                <td class="py-2 text-right font-mono text-slate-500 dark:text-gray-500 text-xs">${displayPrice}</td>
    ...
    function renderStatusBar(dataDate, dataStatus) {
        ...
        if (!dataDate || !dataStatus) {
            statusBar.innerHTML = `<span class="text-3xs text-slate-600 dark:text-gray-600">狀態載入中...</span>`;
            return;
        }
        ...
        const dots = items.map(item => {
            ...
            const color = ok ? "text-emerald-500 dark:text-emerald-600" : "text-slate-500 dark:text-slate-400";
        ...
        statusBar.innerHTML = `
            ...
            ${dots.join('<span class="text-slate-700 dark:text-slate-300 mx-0.5">|</span>')}
        `;
    }
